...
{
GlobalPlannerWithOrientations::GlobalPlannerWithOrientations() : global_planner::GlobalPlanner::GlobalPlanner(), initialized_(false)
{
}

GlobalPlannerWithOrientations::GlobalPlannerWithOrientations(std::string name, costmap_2d::Costmap2D *costmap, std::string frame_id)
...
void GlobalPlannerWithOrientations::initializeThis(std::string name, costmap_2d::Costmap2D *costmap, std::string frame_id)
{
    if (!initialized_) {
        initialized_ = true;
...
bool GlobalPlannerWithOrientations::makePlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal,
...
void GlobalPlannerWithOrientations::filterPlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal,
                                 std::vector< geometry_msgs::PoseStamped > &plan)
{
    ...
    if (plan.size() < max_omni_poses_ * 2 + 2) {
        ...
        for (unsigned int i = 1; i <= max_omni_poses_; ++i) {
            geometry_msgs::Pose *pose = &plan[i].pose;
            weight = (double) i / (double) max_omni_poses_;
        ...
        for (unsigned int i = plan.size() - max_omni_poses_ - 1; i < plan.size() - 1; ++i) {
            geometry_msgs::Pose *pose = &plan[i].pose;
            weight = (double)(i - (plan.size() - max_omni_poses_ - 1)) / (double) max_omni_poses_;
...
double GlobalPlannerWithOrientations::calcOmniRotation(const double from, const double way1, const double way2, const double weight)
{
    if (way1 < way2) {
...
double GlobalPlannerWithOrientations::calcRotation(const geometry_msgs::PoseStamped &from, const geometry_msgs::PoseStamped &to)
{
...
double GlobalPlannerWithOrientations::calcDistance(const geometry_msgs::PoseStamped &pose1, const geometry_msgs::PoseStamped &pose2)
{
...
void GlobalPlannerWithOrientations::publishVisualPlan(const geometry_msgs::PoseStamped &start, const geometry_msgs::PoseStamped &goal,
                                        std::vector< geometry_msgs::PoseStamped > &plan)
{
