/*
 * collision_velocity_filter_node.cpp
 *
        ...
        footprint_msg_received(false), laser_scans_as_pcl_cloud_received(false), desired_twist_msg_received_(false), debug_mode_(false)
{
    ...
    target_frame_ = "/base_link";   //ToDo: read as a parameter
    ...
    sub_footprint_ = nh.subscribe < geometry_msgs::PolygonStamped > (footprint_topic_name, 1, &CollisionVelocityFilterNode::footprintCallback, this);
    ...
    {
        ROS_WARN_STREAM("Could not read parameter <" << footprint_parameter_name << "> from the parameter server. Try to receive it from the specified topic ...");
    ...
    collision_velocity_filter_->setVelocitiesInSoftPadding(linear_velocity_in_soft_padding, angular_velocity_in_soft_padding);
...
bool CollisionVelocityFilterNode::getFootprintFromParameterServer(const std::string &parameter_name)
{
    ...
    // ToDo: read these values also as parameters
...
void CollisionVelocityFilterNode::twoSynchronizedLaserscanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_1, const sensor_msgs::LaserScan::ConstPtr &scan_2)
{
...
void CollisionVelocityFilterNode::threeSynchronizedLaserscanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_1, const sensor_msgs::LaserScan::ConstPtr &scan_2,
...
void CollisionVelocityFilterNode::update()
{
    ...
    {
        safe_twist_ = collision_velocity_filter_->calculateSafeBaseVelocities(desired_twist_msg_, laser_scans_as_pcl_cloud);
